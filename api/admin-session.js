...
export default async function handler(req,res){
  ...
  if(!expected||supplied!==expected){ try{await db('audit_logs',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({action:'LOGIN_FAILED',target_type:'session',actor_ip:ip,metadata:{userAgent:String(req.headers['user-agent']||'').slice(0,300)}})})}catch{} return res.status(401).json({error:'Secret admin salah'}); }
  const token=await issueToken();
  try{await db('audit_logs',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({action:'LOGIN_SUCCESS',target_type:'session',actor_ip:ip,metadata:{userAgent:String(req.headers['user-agent']||'').slice(0,300)}})})}catch{}
